        ...
        try {
            ...
            if (isset($this->parms['ispos']) && !empty($this->parms['ispos']) || isset($this->parms['is_add_sale']) && !empty($this->parms['is_add_sale'])) {
                ...
                if($shippingMethodPrice){
                    ...
                    } elseif ($shippingMethodPrice->methods_type_link == 'shipmentwithcity') {

                        $getCity = City::where('name', $this->parms['delivery_city'])->first();

                        if ($getCity) {
                            $getShipmentPrice = ShippmentWithCity::where('shipping_method_id', 5)
                                                                ->where('city_id', $getCity->id)
                                                                ->first();
                            $shipping_method_price = $getShipmentPrice->shippment_price;
                        } else {
                            $shipping_method_price = 0;
                        }
        ...
        if ($sql) {
            ...
            if($this->parms['payment_method'] == 'stripe'){
                ...
                if($paymentMethod['message'] == 'Success'){
                    $orderService->orderTransectionCredit($sql);
                ...
                try {
                    ...
                    if ($response->isSuccessful()) {
                        ...
                        $paymentMethod['message'] = $response->getMessage();
                    }
                } catch(Exception $e) {
            ...
            else if($this->parms['payment_method'] == 'wallet'){
                $paymentMethod = $orderService->payWithWallet($sql, $this->parms['payment_method'], $this->parms['order_price']);
