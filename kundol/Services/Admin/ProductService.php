...
class ProductService
{
    ...
    {
        //dd ($parms['combination_detail'][0]['product_combination_id']);
        foreach ($data as $j => $result) {
            ...
            if ($type == 'store') {
                ...
                if (isset($parms['combination_detail'][$j]['product_combination_id'])) {
                    $productCombinationData = ProductCombination::where('id', $parms['combination_detail'][$j]['product_combination_id'])->first();
                    if ($productCombinationData) {
                        $this->updateProductCombination($parms[$price], $parms[$gallary], $parms['combination_detail'][$j]['product_combination_id'], $parms[$sku]); 
                    }
                }
    ...
    public function updateProductCombination($price, $gallary, $id, $sku)
    {
        \DB::table('product_combination')
        ->where('id', $id)
        ->limit(1)
        ->update(array('sku' => $sku, 'price' => $price, 'gallary_id' => $gallary));
    }
