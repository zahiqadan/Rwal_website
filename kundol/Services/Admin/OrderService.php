...
class OrderService
{
    ...
    public function CheckStock($customer_id, $warehouse_id)
    {
        ...
        foreach ($sql as $data) {
            ...
            if (!Gate::allows('isDigital')) {
                ...
                $qtyValidation = $qtyValidation->availableQty($data->product_id, $data->product_combination_id, $data->qty, $warehouse_id);
                if (!$qtyValidation) {
                    $error = true;
                    $k = $i;
                    return $this->errorResponseArray('Product Out of Stock at position '.($k), 422);
                    break;
                }
    ...
    public function orderDetail($sql, $order)
    {
        ...
        foreach ($sql as $data) {
            ...
            $orderDetail = OrderDetail::create($parms);
            ...
            $products[] = $parms['product_discount'] > 0 ? $parms['product_discount'] * $parms['qty'] : $parms['product_price'] * $parms['qty'];
    ...
    public function orderTransectionCredit($order)
    {
        $transaction_id = $this->saveTransaction("sale product");
    ...
    public function payWithWallet($order, $payment_method, $amount)
    {        
        if ($payment_method == 'wallet') {

            try {
                $getTotalFromDebitAndCredit = TransactionDetail::GetTotalFromDebitAndCredit();
                if ($getTotalFromDebitAndCredit >= $amount) {
                    $dedicated_amount = $getTotalFromDebitAndCredit - $amount;
                    $status = true;
                } else {
                    $status = false;
                }
                ...
                if ($status == true) {
                    $transaction_id = $this->saveTransaction("sale product");
                    $this->saveTransactionDetail($order->customer_id, $dedicated_amount, 0, 8, $transaction_id, "sale", 'customer', 'pay with wallet amount', null);
                    $this->saveTransactionDetail($order->customer_id, 0, $dedicated_amount, 8, $transaction_id, "sale", 'customer', 'dedicated amount from wallet', null);
                    // $this->saveTransactionDetail($order->customer_id, 0, $order->order_price, 8, $transaction_id, "sale", 'customer', null, null);

                    return $this->successResponseArray('', 'Success');
                } else {
                    return $this->errorResponseArray('Your wallet amount is less than total price');
                }
            } catch (Exception $e) {
                return $this->errorResponseArray($e);
            }
    ...
    public function revertOrderProductsQuantity($order)
    {
        ...
        foreach ($orderDetail as $orderdtl) {
            ...
            if ($orderdtl->product_combination_id != null) {
                $this->saveTransactionDetail($refrence_id, averagePriceProductIdBased(null, $refrence_id, 1) * $orderdtl->qty,0, 6, $transaction_id, "sale", $account_type, $order->warehouse_id);
    ...
    public function orderEmail($order)
    {
        ...
        foreach ($products as $pro) {
            $product_name[] = $pro->product->productDetail[0]->title;
            $product_image[] = $pro->product->gallary->name;
            $product_price[] = $pro->product_price;
            $quantity[] = $pro->qty;
        }

        $order_email_template = Setting::where('key',['order_email_template'])->pluck('value');
        $order_email_template = $order_email_template[0];

        $search = array('Status','order_id','buyer_name','shipping_address','amount');

        $replace = array($order->order_status, $order->id, $order->billing_first_name, $order->delivery_street_aadress, $order->order_price);

        $order_email_template_with_data = str_replace($search, $replace, $order_email_template);
        
        $table_row = '';
        for ($i=0; $i < count($product_name) && $i < count($product_price) && $i < count($product_image) && $i < count($quantity); $i++) {

                $table_row .= '<tr style="border-bottom: 1px solid rgba(0,0,0,.05);"><td valign="middle" width="80%" style="text-align:left; padding: 0 2.5em;"><div class="product-entry"><img src="'.url('/gallary/'.$product_image[$i]).'" alt="" style="width: 100px; max-width: 600px; height: auto; margin-bottom: 20px; display: block;"><div class="text"><h3>'.$product_name[$i].'</h3><p>'.$quantity[$i].'</p></div></div></td><td valign="middle" width="20%" style="text-align:left; padding: 0 2.5em;"><span class="price" style="color: #000; font-size: 20px;">'.$product_price[$i].'</span></td></tr>';
            }


        $order_email_template_with_data = str_replace('cart_items', $table_row, $order_email_template_with_data);

        $message = "";
        $email = Auth::guard('customer-api')->user()->email;

        // Mail::send('emails.order-email-template', ['order_email_template_with_data' => $order_email_template_with_data], function ($message) use ($email) {
        //         $message->to($email)
        //                 ->subject('Order');
        // });

        dispatch(new OrderMail($email, $order_email_template_with_data));
    }
}
