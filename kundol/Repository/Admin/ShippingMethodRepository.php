...
class ShippingMethodRepository implements ShippingMethodInterface
{
    ...
    public function shippingWithCity(array $parms)
    {
        try {
            $shippingMethod = ShippingMethod::where('is_default', '1')->first();
            if ($shippingMethod->methods_type_link == 'shipmentwithcity') {
                $getCity = City::where('name', $parms['delivery_city'])->first();
                if ($getCity) {
                    $getShipmentPrice = ShippmentWithCity::where('shipping_method_id', 5)->where('city_id', $getCity->id)->first();
                    $shipping_method_price = $getShipmentPrice->shippment_price;
                    return response()->json([
                        'status' => 'Success',
                        'shipping_cost' => $shipping_method_price,
                    ], 200);
                } else {
