    ...
    public function all()
    {
        try {
            if (isset($_GET['app_setting'])) {
                $setting = Setting::where('type', 'app_general')
                ->orWhere('type', 'app_login_signup')
                ->orWhere('type', 'point_setting')
                ->orWhere('type', 'business_general')
                ->orWhere('type', 'wallet_setting')
                ->orWhere('type', 'email_template')
                ->get();
            ...
            if (isset($_GET['getSameBilling']) && $_GET['getSameBilling'] == '1') {
                $setting = Setting::where('key', 'same_for_billing')->firstOrFail();
                return $this->successResponse(new SettingResource($setting), 'Data Get Successfully!');
            }
