    ...
    public function store(array $parms)
    {
        foreach ($parms['product_id'] as $i => $product_id) {
            if (isset($parms['warehouse_id'][$i]) && isset($parms['stock_status'][$i]) && isset($parms['qty'][$i]) && $parms['qty'][$i] > 0) {
                ...
                if ($parms['warehouse_id'][$i] != null && $parms['stock_status'][$i] != null && $parms['qty'][$i] != 0 && $parms['stock_status'][$i] == 'OUT') {

                    $product_combination_id = '';

                    if (isset($parms['product_combination_id'][$i])) {
                        $product_combination_id = $parms['product_combination_id'][$i];
                    }

                    $qtyValidation = new AvailableQty;
                    $qtyValidation = $qtyValidation->availableQty($product_id, $product_combination_id, $parms['qty'][$i], $parms['warehouse_id'][$i]);
                    if (!$qtyValidation) {
                        return $this->errorResponse('Product Out of Stock at position '.($i), 422);
                    }
                }
            }
        }
                ...
                if (isset($parms['warehouse_id'][$i]) && isset($parms['stock_status'][$i]) && isset($parms['qty'][$i]) && $parms['qty'][$i] > 0) {
                    ...
                    if ($parms['warehouse_id'][$i] != null || $parms['stock_status'][$i] != null || $parms['qty'][$i] != 0) {
                        ...
                        if (strtolower($parms['stock_status'][$i]) == 'in') {
                            $this->saveTransactionDetail($refrence_id, $parms['price'][$i] * $parms['qty'][$i], 0, 6, $transaction_id, "adjustment", $account_type, $parms['warehouse_id'][$i], 'StockAdjustment');
                        }

                        if (strtolower($parms['stock_status'][$i]) == 'out') {
                            $this->saveTransactionDetail($refrence_id, 0, $parms['price'][$i] * $parms['qty'][$i],  6, $transaction_id, "adjustment", $account_type, $parms['warehouse_id'][$i], 'StockAdjustment');
                        }
                    }
                }
            }
