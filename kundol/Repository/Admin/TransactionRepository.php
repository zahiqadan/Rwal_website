...
class TransactionRepository implements TransactionInterface
{
    ...
    public function store(array $parms)
    {
        try {
            ...
            $refernce_id = Account::where('id', $parms['account_id'])->pluck('reference_id');
            $sql = TransactionDetail::create([
                'transaction_id' => $trans_id->id,
                'account_id' => $parms['account_id'],
                'description' =>$parms['description'],
                'reference_id' => $refernce_id[0],
                'user_id' => 0,
                'type' => '',
        ...
        if ($sql) {
            return $this->successResponse(new TransactionDetailResource($sql), 'Accounts Save Successfully!');
        } else {
            return $this->errorResponse();
        }
    }
