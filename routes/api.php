...
Route::group(['prefix' => 'admin', 'middleware' => ['auth:user-api', 'scopes:user']], function () {
    ...
    Route::resource('state', 'API\Admin\StateController', ['names' => ['index' => 'admin.state.index', 'store' => 'admin.state.store', 'update' => 'admin.state.update', 'destroy' => 'admin.state.destroy']])->only(['index', 'show', 'store', 'update', 'destroy']);

    Route::resource('city', 'API\Admin\CityController', ['names' => ['index' => 'admin.city.index', 'store' => 'admin.city.store', 'update' => 'admin.city.update', 'destroy' => 'admin.city.destroy']])->only(['index', 'show', 'store', 'update', 'destroy']);

    Route::resource('shippmentWithCity', 'API\Admin\ShippmentWithCityController', ['names' => ['index' => 'admin.city.index', 'store' => 'admin.city.store', 'update' => 'admin.city.update', 'destroy' => 'admin.city.destroy']])->only(['index', 'show', 'store', 'update', 'destroy']);
    ...
    Route::post('set-timezone', 'API\Admin\TimezoneController@setTimezone');
    ...
    Route::post('updateLocation', 'API\Admin\BusinessSettingController@updateLocation');
    ...
    Route::resource('importexport', 'API\Admin\ImportExportController', ['names' => ['store' => 'admin.importexport.store']])->only(['store']);

    Route::get('export', 'API\Admin\ImportExportController@export');
    Route::get('sample', 'API\Admin\ImportExportController@sample');
    Route::post('verifyPurchaseCode', 'API\Admin\ImportExportController@verifyPurchaseCode');
    ...
    Route::get('/newslettercontact', 'API\Web\NewsletterContactController@index')->name('newslettercontact.index');
    Route::delete('/newslettercontact/{id}', 'API\Web\NewsletterContactController@destroy')->name('newslettercontact.destroy');
    Route::get('/newslettercontact-csv', 'API\Web\NewsletterContactController@get_csv')->name('newslettercontact.get_csv');
});
...
Route::group(['prefix' => 'client', 'middleware' => ['auth:customer-api', 'scopes:customer']], function () {
    ...
    Route::get('/checkShippingMethod', 'API\Admin\ShippingMethodController@shippingMethod')->name('shipping_method.checkShippingMethod');
    Route::post('/shippingWithCity', 'API\Admin\ShippingMethodController@shippingWithCity')->name('shipping_method.shippingWithCity');
    Route::post('paystack-authorization', 'API\Web\OrderController@paystackAuthorization');
    Route::get('get-fygaro-button', 'API\Web\OrderController@getfygaroButton');
    Route::post('fygaro-jwt', 'API\Web\OrderController@makeFygaroJWT');
    ...
    Route::get('/wallet', 'API\Web\WalletController@index')->name('customer.wallet');
...
Route::group(['prefix' => 'client', 'middleware' => ['checkClientCredentials']], function () {
    ...
    Route::post('/newslettercontact', 'API\Web\NewsletterContactController@store')->name('newslettercontact.store');
});
